...
const imagePlaceholder = "https://images.unsplash.com/photo-1541643600914-78b084683601?q=80&w=600&auto=format&fit=crop";
...
const ProductList: React.FC<ProductListProps> = ({ onProductSelect, onQuickView }) => {
  ...
  const handleAddToCart = (e: React.MouseEvent, product: Product) => {
    ...
    else showToast(`Cannot add more. Max stock reached.`, 'info');
  };
        ...
        <div className="flex flex-col mb-12 gap-8">
          <div className="flex flex-col md:flex-row items-end md:items-center justify-between gap-6 opacity-0 animate-fade-in" style={{ animationDelay: '0.1s' }}>
            ...
            <div className="relative w-full md:w-80 group">
              ...
              />
              <svg className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 dark:text-gray-500 w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-white/40 dark:bg-dark-card/50 p-2 rounded-2xl backdrop-blur-sm border border-white/50 dark:border-white/10 shadow-sm opacity-0 animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 ${
                      selectedCategory === cat 
                        ...
                        : 'bg-white dark:bg-white/5 text-gray-600 dark:text-gray-300 hover:bg-pink-50 dark:hover:bg-white/10'
                    }`}
                  >
                    {cat}
                  </button>
             ...
             <div className="flex items-center gap-2 w-full md:w-auto justify-end">
                ...
                <select 
                  ...
                  className="bg-white dark:bg-white/5 border-none text-sm font-medium text-gray-700 dark:text-gray-200 py-2 pl-4 pr-8 rounded-full focus:ring-0 cursor-pointer"
                >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            {filteredProducts.map((product, index) => {
              ...
              return (
                ...
                >
                  <div 
                    className="relative w-full aspect-[3/4] bg-white dark:bg-dark-card rounded-3xl p-6 flex items-center justify-center z-10 transition-all duration-500 group-hover:-translate-y-3 group-hover:shadow-2xl cursor-pointer shadow-md overflow-hidden"
                  ...
                  >
                     <img 
                      ...
                      alt={product.name} 
                      className={`h-full w-full object-contain drop-shadow-md transition-all duration-700 group-hover:scale-110 z-10 ${isOutOfStock ? 'grayscale opacity-70' : ''}`}
                     />
                     {isOutOfStock && (
                     ...
                     {!isOutOfStock && (
                       <div className="absolute inset-x-0 bottom-0 p-4 opacity-0 group-hover:opacity-100 transition-all duration-300 z-20 translate-y-4 group-hover:translate-y-0 flex flex-col gap-2">
                          <button 
                            ...
                            className="w-full bg-white/90 backdrop-blur-sm text-gray-800 py-2.5 rounded-xl font-medium shadow-lg hover:bg-white flex items-center justify-center gap-2 text-sm"
                          >
                             Quick View
                          </button>
                          <button 
                            onClick={(e) => handleAddToCart(e, product)}
                            className="w-full bg-brand-pink/90 backdrop-blur-sm text-white py-2.5 rounded-xl font-medium shadow-lg shadow-brand-pink/30 hover:bg-brand-pink flex items-center justify-center gap-2 text-sm"
                          >
                             Add to Cart
                          </button>
                       </div>
                  ...
                  <div className="mt-5 text-center px-2">
                     ...
                     <h3 
                       className="font-bold text-gray-900 dark:text-gray-100 text-lg leading-tight mb-1 cursor-pointer hover:text-brand-pink transition-colors"
                       onClick={() => onProductSelect(product)}
                     >
                       {product.name}
                     </h3>
