...
const ImageAnalyzer: React.FC<ImageAnalyzerProps> = ({ onProductSelect }) => {
  const { products, addToCart, addToRecentlyViewed } = useShop();
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      setAnalysis(result);
      // Track recommendations as "seen" potentially, or wait for click
    } catch (error) {
      console.error(error);
    } finally {
      setLoading(false);
  ...
  const renderProductCard = (productId: string) => {
    ...
    return (
      <div key={productId} className="flex flex-col bg-white dark:bg-dark-card rounded-2xl border border-gray-100 dark:border-white/10 overflow-hidden shadow-sm group animate-pop">
        <div className="h-32 bg-gray-50 dark:bg-black/40 flex items-center justify-center p-3 relative overflow-hidden">
        ...
        <div className="p-3">
          <h4 className="text-xs font-bold text-gray-900 dark:text-white truncate mb-1">{product.name}</h4>
          <p className="text-xs font-bold text-brand-pink mb-3">₵{product.price.toLocaleString()}</p>
      ...
      <div className="max-w-6xl mx-auto px-6">
        <div className="flex flex-col lg:flex-row gap-12 items-start">
          <div className="w-full lg:w-1/2">
             ...
             <div className="bg-pink-50 dark:bg-black/20 p-8 rounded-[2.5rem] border border-pink-200 dark:border-white/10 shadow-sm transition-colors">
                <input type="file" accept="image/*" ref={fileInputRef} onChange={handleFileChange} className="hidden" />
                {!selectedImage ? (
                  <div onClick={() => fileInputRef.current?.click()} className="bg-white/80 dark:bg-transparent border-2 border-dashed border-brand-pink/50 rounded-3xl h-72 flex flex-col items-center justify-center cursor-pointer hover:bg-white dark:hover:bg-white/5 hover:border-brand-pink transition-all group shadow-sm">
                  ...
                  <div className="relative h-72 rounded-3xl overflow-hidden shadow-xl bg-gray-100 dark:bg-black/40 border border-white/20">
                     <img src={`data:${mimeType};base64,${selectedImage}`} alt="Preview" className="w-full h-full object-cover" />
                     <button onClick={() => { setSelectedImage(null); setAnalysis(null); }} className="absolute top-4 right-4 bg-black/50 text-white p-2 rounded-full hover:bg-black/70 backdrop-blur-md transition-colors"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg></button>
                  </div>
                )}
                <div className="mt-8">
                  <Button onClick={handleAnalyze} disabled={!selectedImage || loading} className="w-full py-5 text-lg font-serif tracking-widest">
                    {loading ? "CONSULTING FLORA..." : "ANALYZE VIBES"}
                  </Button>
                </div>
             </div>
          </div>
          ...
          <div className="w-full lg:w-1/2 min-h-[500px] flex flex-col">
             {!analysis ? (
               ...
               <div className="animate-fade-in flex flex-col h-full bg-white dark:bg-dark-card p-10 rounded-[2.5rem] shadow-sm border border-gray-100 dark:border-white/10">
                 <div className="mb-8">
                    <h4 className="text-[10px] uppercase tracking-[0.3em] font-bold text-brand-pink mb-4 flex items-center gap-3"><span className="w-8 h-[1px] bg-brand-pink"></span>Olfactory Essence</h4>
                    <div className="text-gray-800 dark:text-gray-200 text-xl font-light leading-relaxed italic border-l-2 border-brand-pink/20 pl-6">
                      {formatText(analysis.description)}
                    </div>
                 </div>
                 {analysis.recommendedProductIds && analysis.recommendedProductIds.length > 0 && (
                   <div className="mt-auto">
                      <h4 className="text-[10px] uppercase tracking-[0.3em] font-bold text-gray-400 mb-6 flex items-center gap-2"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"></path><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"></path></svg>Signature Matches</h4>
