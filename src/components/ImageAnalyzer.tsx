...
async function analyzeImage(base64Image: string, mimeType: string): Promise<AnalysisResponse> {
  const res = await fetch('/api/assistant/analyze', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ imageBase64: base64Image, mimeType }),
  });
  if (!res.ok) return { description: '✦ I am having trouble seeing that clearly. Let\'s try another image. ✦', recommendedProductIds: [] };
  return res.json();
}
...
const ImageAnalyzer: React.FC<ImageAnalyzerProps> = ({ onProductSelect }) => {
  ...
  const handleAnalyze = async () => {
    ...
    setLoading(true);
    try {
      const result = await analyzeImage(selectedImage, mimeType);
      setAnalysis(result);
    } catch (error) {
      console.error(error);
    } finally {
